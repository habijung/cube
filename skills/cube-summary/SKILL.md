...
`--copy` 플래그가 지정되면 화면 출력을 생략하고, 요약 내용을 파일 저장 + 시스템 클립보드에 복사합니다. 토큰 절감을 위해 동일 내용의 중복 출력을 방지합니다.

### 실행 절차

1. **`.cube/` 초기화:** `.cube/` 디렉토리가 없으면 생성하고, `.cube/.gitignore`에 `summary.md`가 없으면 추가하십시오:
...
2. **파일 저장:** 요약 내용을 `{project_root}/.cube/summary.md`에 Write합니다. 화면에 마크다운 코드 블록을 출력하지 마십시오.
3. **클립보드 복사:** 다음 Bash 명령으로 크로스 플랫폼 클립보드에 복사합니다:

   ```bash
   if command -v pbcopy &>/dev/null; then
     pbcopy < .cube/summary.md
   elif command -v xclip &>/dev/null; then
     xclip -selection clipboard < .cube/summary.md
   elif command -v xsel &>/dev/null; then
     xsel --clipboard --input < .cube/summary.md
   elif command -v wl-copy &>/dev/null; then
     wl-copy < .cube/summary.md
   elif command -v clip.exe &>/dev/null; then
     clip.exe < .cube/summary.md
   else
     echo "⚠️ No clipboard tool found" >&2; exit 1
   fi
   ```

4. **결과 안내:** 복사 성공 시 "`.cube/summary.md`에 저장 및 클립보드에 복사되었습니다.", 실패 시 에러 메시지를 출력합니다.

> `--copy` 없이 실행하면 기존과 동일하게 화면에 마크다운 코드 블록만 출력합니다 (파일 저장 및 클립보드 복사 없음).

## Guidelines

- **`--copy` 미사용 시:** 출력은 반드시 **언어를 `markdown`으로 지정한 단 하나의 코드 블록**이어야 합니다. 블록 외부에 추가 텍스트를 출력하지 마세요.
- **`--copy` 사용 시:** 화면에 마크다운 코드 블록을 출력하지 마세요. 파일 저장 및 클립보드 복사 후 결과 안내 메시지만 출력하세요.
